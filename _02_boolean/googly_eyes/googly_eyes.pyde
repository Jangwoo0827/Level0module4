...
def draw():
    ...
    draw_googly_eye(312, 214, 50, 80)
    # 7. Repeat the steps above for the right eye and observe the googly eyes!
    fill(255, 255, 255)
    circle(491, 239, 50)
    draw_googly_eye(491, 239, 50, 80)
# ======================= DO NOT MODIFY THE CODE BELOW ========================== it wasn't working, so i fixed code below

def draw_googly_eye(eye_center_x, eye_center_y, eye_diameter, pupil_diameter):
    fill(255, 255, 255)
    if is_mouse_inside_eye(eye_center_x, eye_center_y, eye_diameter/2, pupil_diameter/7):
        circle(mouseX, mouseY, pupil_diameter)
    else:
        position = get_eye_position(eye_center_x, eye_center_y, eye_diameter/2, pupil_diameter/7)
